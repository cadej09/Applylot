        ...
        with sync_playwright() as p:
            ...
            for i, (url, title) in enumerate(jobs):
                ...
                if status in ("ok", "partial"):
                    stats[status] += 1
                    # dupe_sig/company_key are derived here, at the moment the
                    # description lands, so the apply-stage duplicate guard can
                    # see this posting without waiting for a backfill pass.
                    from applypilot.database import company_key, job_dupe_sig
                    _desc = result.get("full_description")
                    conn.execute(
                        "UPDATE jobs SET full_description = ?, application_url = ?, "
                        "detail_scraped_at = ?, detail_error = NULL, "
                        "dupe_sig = ?, company_key = COALESCE(company_key, ?) "
                        "WHERE url = ?",
                        (_desc, result.get("application_url"), now,
                         job_dupe_sig(job.get("title"), _desc),
                         company_key(job.get("company")), url),
                    )
                else:
                    stats["error"] += 1
                    conn.execute(
