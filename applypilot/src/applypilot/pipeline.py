...
def _run_discover(workers: int = 1) -> dict:
    ...
    # Extra boards with free public APIs (RemoteOK, HN Who is Hiring)
    console.print("  [cyan]Extra boards (RemoteOK, HN Who is Hiring)...[/cyan]")
    try:
        from applypilot.discovery.boards import run_boards_discovery
        run_boards_discovery()
        stats["boards"] = "ok"
    except Exception as e:
        log.error("Extra boards failed: %s", e)
        console.print(f"  [red]Extra boards error:[/red] {e}")
        stats["boards"] = f"error: {e}"

    # Smart extract (AI-powered) -- the ONLY discovery step that spends Claude
    # API credits. Off by default so board scraping (JobSpy + Workday, both
    # free HTTP) can be iterated on without cost. Enable with
    # `smart_extract: true` in ~/.applypilot/searches.yaml.
    from applypilot.config import load_search_config
    if load_search_config().get("smart_extract", False):
        ...
        console.print("  [dim]Smart extract disabled (smart_extract: false) — skipping LLM scraping.[/dim]")
