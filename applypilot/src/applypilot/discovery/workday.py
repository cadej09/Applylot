...
def _location_ok(location: str | None, accept: list[str], reject: list[str]) -> bool:
    ...
    # Reject first -- catches foreign "remote" jobs like "Remote, India" or
    # "Bengaluru (Remote)" before the remote shortcut can wave them through.
    for r in reject:
        if r.lower() in loc:
            return False
...
def _ssl_context():
    """SSL context backed by certifi's up-to-date CA bundle.

    The system/OpenSSL default store on this machine rejects many Workday
    tenant certs with 'certificate has expired' (stale root chain). certifi
    ships the current Mozilla bundle and fixes all of them at once.
    """
    import ssl
    try:
        import certifi
...
def _https_handler():
    return urllib.request.HTTPSHandler(context=_ssl_context())
...
def setup_proxy(proxy_str: str | None) -> None:
    ...
    _opener = urllib.request.build_opener(proxy_handler, _https_handler())
...
def store_results(conn: sqlite3.Connection, jobs: list[dict], employers: dict) -> tuple[int, int]:
    ...
    for job in jobs:
        ...
        try:
            # dupe_sig/company_key derived at insert — Workday rows arrive with
            # the description already attached and never reach detail.py, so
            # this is their only chance at a signature. See jobspy.py's note.
            # `site` is the employer name for these rows (employer_name above).
            ...
            conn.execute(
                ...
                (url, job.get("title"), None, short_desc, job.get("location"),
                 ...
                 job_dupe_sig(job.get("title"), full_description), company_key(site)),
            )
            new += 1
...
def _process_one(
    ...
    # Skip detail fetches for jobs already in the DB. Detail fetching is by
    # far the slowest part of the crawl (~1 job/sec), and on a daily cadence
    # 90%+ of search results are already known — checking the DB first cuts
    # the Workday stage from hours to minutes.
    conn = get_connection()
    fresh: list[dict] = []
    known = 0
    for job in jobs:
        ...
        if url and conn.execute(
                "SELECT 1 FROM jobs WHERE url = ? OR application_url = ? LIMIT 1",
                (url, url)).fetchone():
            known += 1
        else:
            fresh.append(job)
    if known:
        log.info("%s: %d already in DB — details skipped; %d new to fetch",
                 emp["name"], known, len(fresh))
    jobs = fresh
